...
Thermostat::Thermostat(std::unique_ptr<RoomInterface> room,
    ...
    : ThermostatInterface::ThermostatInterface(),
      _room(std::move(room)),
...
TemperatureRange Thermostat::getTemperature() const
{
    return _temperature;
}

ThermostatState Thermostat::getState() const
{
    return _state;
}
...
{
    _state = getNewState();
    _timer->configure(std::chrono::seconds(20),
                    [this]() {
...
ThermostatState Thermostat::getNewState() const
{
    ...
    std::cout << "Configured temperature range: " 
                    << _temperature.first << " - "
    ...
    if (_room->getTemperature() < _temperature.first) {
        ...
        std::cout << "Ideal temperature achieved. Systems set to idle." << std::endl;
